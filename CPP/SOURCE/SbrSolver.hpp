...
public:
	...
	void MonostaticRcsGpu(
		const ReducedBvhArray<T>& bvhArray,
		const ObservationArray<T>& obsArray,
		RcsArray<T>& rcsArray,
		size_t maxBatchRays = 10000  // tweak to your GPU’s sweet spot
	) {
		...
		for (U32 i = 0; i < obsCount; ++i) {
			// 1) build the entire ray pool for this observation
			PopulateRayPool(bvhArray, obsPtr[i], rayPool);
			size_t totalRays = rayPool.rayCount_; // = rayCountSqrt_^2

			// 2) launch in subbatches
			size_t offset = 0;
			
			while (offset < totalRays) {
				size_t batch = totalRays - offset;
				if (maxBatchRays < totalRays - offset) {
					batch = maxBatchRays;
				}

				// 2a) slice just [offset .. offset+batch) of your rayPool
				//     ShootAndBounceRaysGpuRange is a small variant that only
				//     builds an array_view of length `batch` at `rayPtr + offset`.
				ShootAndBounceRaysGpuRange(
					bvhArray,
					rayPool,
					offset,
					batch
				...
				offset += batch;
				std::cout << "batch " << offset << "/" << totalRays << std::endl;
			}
			// i need help recreating my raypool here HEREEE

			// 2b) integrate _only_ that batch
			//totalRcs += PhysicalOpticsIntegralRange(
			//	rayPool,
			//	obsPtr[i],
			//	offset,
			//	batch
			...
			PhysicalOpticsIntegral(rayPool, obsPtr[i], rcsPtr[i]);

			// 3) store the combined result
			
			std::cout << i << " / " << obsCount << std::endl;
		}
	}
	...
	template<typename T>
	void ShootAndBounceRaysGpuRange(
		const ReducedBvhArray<T>& bvhArray,
		RayPool<T>& rayPool,
		size_t offset,    // which ray index to start at
		size_t count      // how many rays to process
	) {
		using namespace concurrency;
		// grab and hold the one view

		// host pointers
		ReducedBvhNode<T>* bvhPtr = bvhArray.bvhNodeArray_.get();
		RayTube< T >* rayPtr = rayPool.rayTubeArray_.get() + offset;


		// BVH view (full BVH, always all nodes)
		array_view<const ReducedBvhNode<T>, 1> bvhGpu(bvhArray.nodeCount_, bvhPtr);

		array_view<RayTube<T>, 1> rayGpu(count, rayPtr);
		...
		// copy this slice back
		rayGpu.synchronize();
		/*
		for (int i = 0; i < 2750 ; ++i) {
			auto& r = rayPool.rayTubeArray_.get()[i];
			if (r.refCount_ > 0) {
	...
	T PhysicalOpticsIntegralRange(const RayPool< T >& rayPool, const Observation< T >& obs, size_t offset, size_t count)
	{
		U32 totalRays = rayPool.rayCount_;
		T rcs = 0;
		using namespace std;
		...
		for (U32 idRay = offset; idRay < offset + count; ++idRay)
		{
	...
	/*
		void MonostaticRcsGpu(const ReducedBvhArray< T >& bvhArray, const ObservationArray< T >& obsArray, RcsArray< T >& rcsArray)
		{
			...
			for (U32 idx = 0; idx < obsCount; ++idx)
			{
				...
				std::cout << idx << " / " << obsCount << std::endl;
			}
		}
		...
		void ShootAndBounceRaysGpu(const ReducedBvhArray< T >& bvhArray, RayPool< T >& rayPool)
		{
			...
			for (int i = 0; i < 3500; ++i) {
				auto& r = rayPool.rayTubeArray_.get()[i];
				if (r.refCount_ > 0) {
					std::cout << "Ray[" << i << "] dist=" << r.dist_
						<< " refCount=" << r.refCount_
